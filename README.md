# Felipe Silva de Freitas

## Estudante de Análise e Desenvolvimento de Sistemas (ADS) | Em busca do primeiro estágio

### Sobre Mim
Olá! Sou o Felipe, estudante do 2º semestre de Análise e Desenvolvimento de Sistemas (ADS) no Centro Universitário Módulo. Estou no início da minha jornada na tecnologia e com muita vontade de aprender e crescer na área. Até agora, tive meus primeiros contatos com lógica de programação usando Python e aprendi o básico sobre como desenhar interfaces pensando na experiência do usuário. 

Neste momento, estou dando meus primeiros passos em Desenvolvimento Front-End (HTML/CSS) e entendendo como funcionam os Bancos de Dados. Meu maior objetivo hoje é conseguir o meu primeiro estágio, um ambiente onde eu possa aprender com profissionais mais experientes, colocar os exercícios da faculdade em prática e evoluir todos os dias.

### Habilidades Técnicas
*   **Desenvolvimento Front-End:** Estruturação para Web (HTML, CSS), Design Profissional
*   **Programação & Lógica:** Python (listas, tuplas, dicionários, NumPy), Algoritmos e Pensamento Computacional
*   **Dados:** Modelagem e estruturação de Banco de Dados
*   **UX/UI & Design:** Figma, Miro, Lucidchart, Interação Humano-Computador, Acessibilidade (WCAG)
*   **Inteligência Artificial:** Engenharia de Prompt e aplicações em IA
*   **Ferramentas Base:** Git, GitHub, Markdown

### Projetos e Estudos em Destaque
*   **[EventCaraguá - Plataforma de Eventos Locais (Em Desenvolvimento)](COLOQUE_O_LINK_DO_REPOSITORIO_AQUI)**
    Desenvolvendo em dupla um sistema web para divulgação e inscrições de eventos da cidade. Atuo na estruturação do desenvolvimento Front-End e na modelagem evolutiva do banco de dados relacional, aplicando conceitos práticos semanais conforme o avanço do projeto.
*   **[Resolução de Algoritmos em Python](#)**
    Desenvolvimento de pequenos scripts utilizando a lógica de programação, listas e tuplas para resolver problemas propostos na disciplina de Pensamento Computacional.

### Contato
*   **LinkedIn:** [linkedin.com/in/felipe-silva](https://www.linkedin.com/in/felipe-silva-b66b80373/)
*   **E-mail:** [felipefreitass.tech@gmail.com](mailto:felipefreitass.tech@gmail.com)
*   **GitHub:** [github.com/FelipeFreitas5](https://github.com/FelipeFreitas5)
